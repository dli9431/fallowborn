...
STRUCTURED_DATA = {
    ...
    "ailments": "ailment",
    "modifiers": "modifier",
    "cultureTraditions": "cultureTradition",
    "cultures": "culture",
    "religions": "religion",
    "buildings": "building",
    "fortLevels": "fort",
    ...
    "marketEndowmentTypes": "marketEndowment",
    ...
    for data_name, namespace in STRUCTURED_DATA.items():
        ...
        if data_name in ("techDomains", "techTraditions"):
            ...
            path = DATA / ("traits.js" if data_name in ("traits", "ailments") else
                           ...
                           "cultures.js" if data_name in ("cultureTraditions", "cultures", "religions") else
                           ...
                           "policies.js" if data_name == "policies" else
                           "political_institutions.js" if data_name in (
                               "councilSeats", "privileges"
                           ) else
                           "map_data.js")
...
def render_catalog(
    ...
    catalog_slot = (
        f"FBDATA.lang.{code}" if re.fullmatch(r"[A-Za-z_$][A-Za-z0-9_$]*", code)
        else f"FBDATA.lang[{js_string(code)}]"
    )
    lines = [
...
def locale_words(code: str) -> dict[str, dict[str, str]]:
    tables = {
        ...
        "pt-BR": {
            "child": {"m": "filho", "f": "filha", "x": "filho(a)"},
            "parent": {"m": "pai", "f": "mãe", "x": "pai/mãe"},
            "sibling": {"m": "irmão", "f": "irmã", "x": "irmão(ã)"},
            "spouse": {"m": "marido", "f": "esposa", "x": "cônjuge"},
            "monarch": {"m": "rei", "f": "rainha", "x": "monarca"},
            "noble": {"m": "senhor", "f": "senhora", "x": "nobre"},
            "pronoun": {"m": "ele", "f": "ela", "x": "ele(a)"},
            "possess": {"m": "dele", "f": "dela", "x": "dele(a)"},
        },
    }
...
def main() -> int:
    ...
    translate.add_argument("locales", nargs="+", choices=("fr", "de", "it", "es", "pt-BR"))
    translate.set_defaults(func=command_translate)
    validate = sub.add_parser("validate")
    validate.add_argument("locales", nargs="*", default=("fr", "de", "it", "es", "pt-BR"))

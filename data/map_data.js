...
FBDATA.buildings = {
  ...
  temple:  { name:'Great {temple}', icon:'🛐', cost:70, upkeep:1, piety:2, pop:5, populationCrisisProtection:0.10, communityFaithPressure:0.12, requiresTech:'lime_mortar',
  ...
  cathedral: { name:{ default:'Cathedral', muslim:'Great Friday Mosque', pagan:'Grand Sanctuary' }, icon:'⛪', cost:240, upkeep:3, devMin:7, maxCounty:1, piety:4, prestige:15, pop:10, dev:1, populationCrisisProtection:0.15, communityFaithPressure:0.20, requiresTech:'ribbed_vaulting',
...
FBDATA.items = {
  ...
  mail_chausses: { name:'Mail Chausses', icon:'🥾', rarity:'common', value:65,
    unique:false, slot:'feet', ageMin:14, militaryMarket:true,
    requiresTech:'mail_hauberks', yearMin:1200, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.015 }, qualityFx:{ battle:0.005 },
    art:{ kind:'mail_boots', metals:['#939fa6','#b9c3c9'], leathers:['#422e23','#5b402b'] },
    desc:'Riveted mail leggings with foot coverings, worn over hose and leather shoes by mounted knights.' },
  plate_sabatons: { name:'Plate Sabatons', icon:'🥾', rarity:'common', value:130,
    unique:false, slot:'feet', ageMin:14, militaryMarket:true,
    requiresTech:'plate_armor', yearMin:1400, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.03 }, qualityFx:{ battle:0.005 },
    art:{ kind:'plate_boots', metals:['#939fa6','#b9c3c9'], leathers:['#422e23','#5b402b'] },
    desc:'Overlapping steel foot defenses over arming shoes, paired with the late-medieval plate harness.' },
  knightly_bascinet: { name:'Knightly Bascinet', icon:'⛑', rarity:'common', value:150,
    unique:false, slot:'head', ageMin:14, militaryMarket:true,
    requiresTech:'plate_armor', yearMin:1400, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.04 }, qualityFx:{ battle:0.005 },
    art:{ kind:'bascinet', metals:['#939fa6','#b9c3c9'], leathers:['#422e23','#5b402b'] },
    desc:'A visored steel helmet with a mail neck defense, completing a late-medieval knightly harness.' },
  knightly_mail: { name:'Knightly Mail Hauberk', icon:'🛡', rarity:'common', value:110,
    unique:false, slot:'body', ageMin:14, militaryMarket:true,
    requiresTech:'mail_hauberks', yearMin:1200, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.05 }, qualityFx:{ battle:0.005 },
    art:{ kind:'mail', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'A long riveted mail coat: the principal body defense of medieval western knights.' },
  lamellar_cuirass: { name:'Lamellar Cuirass', icon:'🛡', rarity:'common', value:95,
    unique:false, slot:'body', ageMin:14, militaryMarket:true,
    requiresTech:'scale_lamellar', yearMin:867, cultures:['greek', 'armenian', 'georgian', 'persian', 'turkic', 'khazar', 'magyar', 'rus'],
    fx:{ battle:0.05 }, qualityFx:{ battle:0.005 },
    art:{ kind:'lamellar', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'Small overlapping plates laced together, in the traditions of Byzantine and steppe armorers.' },
  knights_plate: { name:'Knight’s Plate Armor', icon:'🛡', rarity:'common', value:340,
    unique:false, slot:'body', ageMin:14, militaryMarket:true,
    requiresTech:'plate_armor', yearMin:1400, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.08 }, qualityFx:{ battle:0.01 },
    art:{ kind:'plate', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'An articulated steel harness in the western European tradition, worn over padding with mail at the gaps. The helmet and armored footwear are equipped separately.' },
  mail_coif: { name:'Mail Coif', icon:'⛑', rarity:'common', value:55,
    unique:false, slot:'head', ageMin:14, militaryMarket:true,
    requiresTech:'mail_hauberks', yearMin:1200, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ battle:0.02 }, qualityFx:{ battle:0.005 },
    art:{ kind:'coif', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'A hood of linked iron rings protecting the neck and head of a western knight.' },
  knightly_lance: { name:'Knightly Lance', icon:'⚔', rarity:'common', value:70,
    unique:false, slot:'hand', ageMin:14, militaryMarket:true,
    requiresTech:'cavalry_lances', yearMin:1050, cultures:['frankish', 'german', 'norman', 'english', 'gaelic', 'brezhon', 'iberian', 'italian', 'lombard', 'occitan', 'basque', 'norse'],
    fx:{ mar:1, battle:0.03 }, qualityFx:{ battle:0.005 },
    art:{ kind:'spear', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'A stout lance intended for the couched charge of western mounted knights.' },
  steppe_bow: { name:'Steppe Composite Bow', icon:'🏹', rarity:'common', value:75,
    unique:false, slot:'hand', ageMin:14, militaryMarket:true,
    requiresTech:'composite_bow', yearMin:867, cultures:['turkic', 'khazar', 'magyar', 'persian'],
    fx:{ mar:1, battle:0.05 }, qualityFx:{ battle:0.01 },
    art:{ kind:'bow', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'Horn, wood, and sinew form the compact recurved bow of the steppe and Iranian mounted archer.', grip:2 },
  crucible_sword: { name:'Crucible-steel Sword', icon:'⚔', rarity:'common', value:125,
    unique:false, slot:'hand', ageMin:14, militaryMarket:true,
    requiresTech:'crucible_steel', yearMin:867, cultures:['persian', 'arabic', 'turkic'],
    fx:{ mar:1, battle:0.025 }, qualityFx:{ battle:0.005 },
    art:{ kind:'sword', metals:['#939fa6','#b9c3c9'], woods:['#68482b'], grips:['#422e23'] },
    desc:'A straight sword forged from crucible steel in the Iranian and Islamic metalworking traditions.' },
  pattern_welded_sword: { name:'Pattern-welded Sword', icon:'⚔', rarity:'common', value:120,
    unique:false, slot:'hand', ageMin:14, militaryMarket:true,
    requiresTech:'pattern_welding', yearMin:867, cultures:['norse','frankish','german','english'],
    fx:{ mar:1, battle:0.02 }, qualityFx:{ battle:0.005 },
    art:{ kind:'sword', metals:['#939fa6','#b9c3c9'], grips:['#422e23'] },
    desc:'Twisted iron and steel bars form a patterned blade in the early-medieval northern European tradition.' },
  padded_jack:     { name:'Padded Jack', icon:'🧥', rarity:'common', value:25,
    unique:false, slot:'body', ageMin:12, fx:{ battle:0.02 }, qualityFx:{ battle:0.005 },
  ...
  nasal_helm:      { name:'Nasal Helm', icon:'⛑', rarity:'common', value:25,
    unique:false, slot:'head', ageMin:12, fx:{ battle:0.02 }, qualityFx:{ battle:0.005 },
  ...
  ash_spear:       { name:'Ash Spear', icon:'🔱', rarity:'common', value:20,
    unique:false, slot:'hand', grip:2, ageMin:12, fx:{ mar:1, battle:0.03 },
  ...
  round_shield:    { name:'Round Shield', icon:'🛡', rarity:'common', value:25,
    unique:false, slot:'hand', ageMin:12, fx:{ battle:0.02 }, qualityFx:{ battle:0.005 },
  ...
  bearded_axe:     { name:'Bearded Axe', icon:'🪓', rarity:'common', value:22,
    unique:false, slot:'hand', ageMin:12, fx:{ mar:1, battle:0.01 }, qualityFx:{ battle:0.005 },
    art:{ kind:'axe', woods:['#7a5531','#5d3d24'], metals:['#b4bcbc','#cdd1cb'] },
    desc:'A hooked blade that pulls shields aside and arguments to a close.' },
  quarterstaff:    { name:'Quarterstaff', icon:'🎋', rarity:'common', value:8,
    unique:false, slot:'hand', grip:2, ageMin:8, fx:{ mar:1 }, qualityFx:{ mar:1 },
    art:{ kind:'staff', woods:['#8a663d','#6b4a2b'] },
    desc:'Six feet of seasoned oak. Masters of it are rarer than swordsmen, and cheaper.' },
  reaping_sickle:  { name:'Reaping Sickle', icon:'⚒', rarity:'common', value:8,
    unique:false, slot:'hand', ageMin:8, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'sickle', woods:['#76502d','#5b3c23'], metals:['#9da4a1','#bdc1ba'] },
    desc:'A short iron crescent for harvest days, sharpened whenever the lord’s acres come due.' },
  irrigation_spade:{ name:'Irrigation Spade', icon:'⛏', rarity:'common', value:9,
    unique:false, slot:'hand', grip:2, ageMin:10, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'spade', woods:['#775332','#604128'], metals:['#858b88','#a6aaa4'] },
    desc:'A narrow spade for opening channels, clearing silt, and turning stubborn earth.' },
  shepherds_crook: { name:'Shepherd’s Crook', icon:'🎋', rarity:'common', value:7,
    unique:false, slot:'hand', grip:2, ageMin:6, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'crook', woods:['#8b673e','#6d4c2d'] },
    desc:'A hooked staff for sheep, cattle, and the long walk between pasture and fold.' },
  woodland_billhook:{ name:'Woodland Billhook', icon:'⚒', rarity:'common', value:10,
    unique:false, slot:'hand', grip:2, ageMin:10, fx:{ mar:1 }, qualityFx:{ ste:1 },
    art:{ kind:'billhook', woods:['#74502f','#563921'], metals:['#8f9693','#b5bab3'] },
    desc:'A pole blade for coppice, deadwood, hedges, and every woodland turn owed.' },
  boat_hook:       { name:'Boat Hook', icon:'⚓', rarity:'common', value:9,
    unique:false, slot:'hand', grip:2, ageMin:10, fx:{ mar:1 }, qualityFx:{ ste:1 },
    art:{ kind:'hook', woods:['#775231','#5b3d25'], metals:['#8f9694','#b9bdb7'] },
    desc:'A long pole and iron hook for landing boats, shifting cargo, and working a rough shore.' },
  hand_quern:      { name:'Hand Quern', icon:'⚙', rarity:'common', value:10,
    unique:false, slot:'hand', grip:2, ageMin:8, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'quern', stones:['#77736a','#918b7d','#68665f'], woods:['#68462a'] },
    desc:'Two fitted stones and a wooden handle, small enough to grind beyond the manor mill.' },
  oven_peel:       { name:'Oven Peel', icon:'🍞', rarity:'common', value:6,
    unique:false, slot:'hand', grip:2, ageMin:8, fx:{ health:0.001 }, qualityFx:{ health:0.001 },
    art:{ kind:'peel', woods:['#8a6238','#6d492b'] },
    desc:'A broad wooden paddle for the common oven, blackened where the loaves meet the heat.' },
  tally_stick:     { name:'Tally Stick', icon:'✎', rarity:'common', value:5,
    unique:false, slot:'hand', ageMin:6, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'tally', woods:['#a47a45','#825c35'] },
    desc:'Notches remember workdays, rent, and dues when a spoken reckoning might not.' },
  gathering_basket:{ name:'Gathering Basket', icon:'❧', rarity:'common', value:6,
    unique:false, slot:'hand', ageMin:6, fx:{ lea:1 }, qualityFx:{ lea:1 },
    art:{ kind:'basket', woods:['#9a713d','#76522e','#b0874c'] },
    desc:'Split withies woven for gleaned grain, woodland forage, and whatever the household may keep.' },
  hunting_bow:     { name:'Hunting Bow', icon:'🏹', rarity:'common', value:24,
    unique:false, slot:'hand', grip:2, ageMin:10, fx:{ mar:1, battle:0.01 }, qualityFx:{ battle:0.01 },
    art:{ kind:'bow', woods:['#7c5a33','#5e3f24'], trims:['#c9bd9a','#a89a76'] },
    desc:'Feeds the pot in peace and feathers a charge in war.' },
  pilgrims_staff:  { name:'Pilgrim’s Staff', icon:'🎋', rarity:'common', value:10,
    unique:false, slot:'hand', grip:2, ageMin:6, fx:{ piety:0.5, health:0.001 }, qualityFx:{ piety:0.5 },
    art:{ kind:'staff', woods:['#8f6c40','#71522f'] },
    desc:'The road is long; lean on something.' },
  money_coffer:    { name:'Money Coffer', icon:'🧰', rarity:'common', value:16,
    unique:false, slot:'hand', ageMin:6, fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'chest', woods:['#5a3a24'], metals:['#8f7a4e'] },
    desc:'A locked box keeps the coin and the count honest.' },
  psalter:         { name:'Psalter', icon:'📖', rarity:'common', value:22,
    unique:false, slot:'hand', ageMin:6, fx:{ piety:0.5 }, qualityFx:{ piety:0.5 },
    art:{ kind:'book', covers:['#4a3a5e','#3c4a58'], pages:['#ddd0a8','#cbbd92'] },
    desc:'A hundred and fifty songs against the dark, thumbed soft at the edges.' },
  war_hammer:      { name:'War Hammer', icon:'🔨', rarity:'fine', value:45,
    unique:false, slot:'hand', ageMin:14, fx:{ mar:1, battle:0.02 }, qualityFx:{ battle:0.01 },
    art:{ kind:'axe', woods:['#6b4a2b'], metals:['#9aa2a2','#7d8484'] },
    desc:'Mail turns a blade. It does not turn this.' },
  falchion:        { name:'Falchion', icon:'🗡', rarity:'fine', value:50,
    unique:false, slot:'hand', ageMin:14, fx:{ mar:2, battle:0.01 }, qualityFx:{ mar:1 },
    art:{ kind:'sword', metals:['#c3caca','#dde1db'], grips:['#3c2a1e','#241a14'] },
    desc:'A single edge, heavy toward the point — a butcher’s honesty in a knight’s hand.' },
  kite_shield:     { name:'Kite Shield', icon:'🛡', rarity:'fine', value:45,
    unique:false, slot:'hand', ageMin:12, fx:{ battle:0.03 }, qualityFx:{ battle:0.01 },
    art:{ kind:'shield', woods:['#4a5a6e','#6e4430'], metals:['#9aa2a2','#c0bfb0'] },
    desc:'Long enough to guard shin as well as shoulder, from horse or on foot.' },
  herbal:          { name:'A Well-Thumbed Herbal', icon:'🌿', rarity:'fine', value:40,
    unique:false, slot:'hand', ageMin:6, fx:{ lea:1, health:0.002 }, qualityFx:{ lea:1 },
    art:{ kind:'book', covers:['#3d5a3a','#4e6b41'], pages:['#ddd0a8','#cbbd92'] },
    desc:'Which leaf feeds, which leaf kills, and which leaf merely tastes of regret.' },
  book_of_hours:   { name:'Book of Hours', icon:'🕯', rarity:'fine', value:55,
    unique:false, slot:'hand', ageMin:6, fx:{ piety:1 }, qualityFx:{ piety:0.5 },
    art:{ kind:'book', covers:['#5e3a4a','#71455a'], pages:['#e2d5ae','#d2c49a'] },
    desc:'Prayers for every hour, painted in gold licked thin as a saint’s patience.' },
  astrolabe:       { name:'Brass Astrolabe', icon:'🧭', rarity:'famed', value:160,
    unique:false, slot:'hand', ageMin:10, fx:{ lea:2, ste:1 }, qualityFx:{ lea:1 },
    art:{ kind:'pendant', metals:['#c8a04a','#a8843c'] },
    desc:'The heavens reduced to rings and rules — useful for calendars, debts, and voyages.' },
  dane_axe:        { name:'Dane Axe', icon:'🪓', rarity:'famed', value:140,
    unique:false, slot:'hand', grip:2, ageMin:14, fx:{ mar:2, battle:0.03 }, qualityFx:{ mar:1, battle:0.01 },
    art:{ kind:'axe', woods:['#5d3d24'], metals:['#c3cbcb','#e0e4de'] },
    desc:'A haft as tall as a man and a blade like a cold moon. Shield walls remember it.' },

  wool_cloak:      { name:'Wool Cloak', icon:'🧣', rarity:'common', value:14,
    unique:false, slot:'body', fx:{ health:0.001 }, qualityFx:{ health:0.001 },
    art:{ kind:'jack', cloths:['#6e5a40','#585048'], threads:['#8a7a5a'] },
    desc:'Rain runs off good wool. So does gossip, mostly.' },
  mail_hauberk:    { name:'Mail Hauberk', icon:'🛡', rarity:'fine', value:70,
    unique:false, slot:'body', ageMin:12, fx:{ battle:0.04 }, qualityFx:{ battle:0.01 },
    art:{ kind:'jack', cloths:['#7d8484','#6a7272'], threads:['#a8b0b0'] },
    desc:'Ten thousand rings, each one riveted, each one a small argument against dying.' },
  fur_mantle:      { name:'Fur Mantle', icon:'🧥', rarity:'fine', value:45,
    unique:false, slot:'body', fx:{ prestige:0.5 }, qualityFx:{ prestige:0.5 },
    art:{ kind:'jack', cloths:['#5e4632','#6e5238'], threads:['#c8b490'] },
    desc:'Warm shoulders read as rich shoulders, and rich shoulders get heard.' },
  lamellar_cuirass:{ name:'Lamellar Cuirass', icon:'⛓', rarity:'famed', value:150,
    unique:false, slot:'body', ageMin:12, fx:{ battle:0.05 }, qualityFx:{ battle:0.01 },
    art:{ kind:'jack', cloths:['#6e6a58','#7d7660'], threads:['#3a3430'] },
    desc:'Laced plates in the eastern fashion, bright as a fish in the sun.' },

  kettle_helm:     { name:'Kettle Helm', icon:'⛑', rarity:'fine', value:45,
    unique:false, slot:'head', ageMin:12, fx:{ battle:0.02 }, qualityFx:{ battle:0.01 },
    art:{ kind:'helm', metals:['#8a9090','#a5aaa7'], trims:['#54402a'] },
    desc:'A wide brim keeps off iron and weather alike.' },
  spangenhelm:     { name:'Spangenhelm', icon:'⛑', rarity:'famed', value:130,
    unique:false, slot:'head', ageMin:12, fx:{ battle:0.03, prestige:0.5 }, qualityFx:{ battle:0.01 },
    art:{ kind:'helm', metals:['#a5aaa7','#c8cdc6'], trims:['#b89042'] },
    desc:'Banded steel in the old imperial style, fit to be recognized across a field.' },

  amber_beads:     { name:'Amber Beads', icon:'📿', rarity:'common', value:15,
    unique:false, slot:'neck', fx:{ dip:1 }, qualityFx:{ dip:1 },
    art:{ kind:'pendant', metals:['#b89042'], gems:['#c8862e','#a86a24'], cords:['#4a3021'] },
    desc:'Sunlight, fossilized and strung. People lean in to look.' },
  torc:            { name:'Noble’s Torc', icon:'⭕', rarity:'fine', value:50,
    unique:false, slot:'neck', fx:{ prestige:1 }, qualityFx:{ prestige:0.5 },
    art:{ kind:'pendant', metals:['#c8a04a','#d8b455'], cords:['#6b4930'] },
    desc:'An open ring of twisted gold — the old sign that a neck answers to no one.' },

  tooled_belt:     { name:'Tooled Belt', icon:'➰', rarity:'fine', value:40,
    unique:false, slot:'waist', fx:{ ste:1 }, qualityFx:{ ste:1 },
    art:{ kind:'belt', leathers:['#5e3a24','#71462a'], metals:['#b3a16d','#c8b678'] },
    desc:'Purse, keys, tallies, knife — a steward’s whole office at one hip.' },
  silk_sash:       { name:'Silk Sash', icon:'🎀', rarity:'famed', value:120,
    unique:false, slot:'waist', fx:{ prestige:1, dip:1 }, qualityFx:{ dip:1 },
    art:{ kind:'belt', leathers:['#7d2e3a','#8e3a46'], metals:['#d0aa45'] },
    desc:'Woven farther east than most men will ever travel, and worth the journey.' },

  furlined_boots:  { name:'Fur-lined Boots', icon:'🥾', rarity:'common', value:15,
    unique:false, slot:'feet', fx:{ health:0.002 }, qualityFx:{ health:0.001 },
    art:{ kind:'boots', leathers:['#4a3226','#5e402c'] },
    desc:'Winter kills from the ground up. These argue otherwise.' },
  riding_boots:    { name:'Riding Boots', icon:'👢', rarity:'fine', value:40,
    unique:false, slot:'feet', fx:{ prestige:0.5 }, qualityFx:{ prestige:0.5 },
    art:{ kind:'boots', leathers:['#3a2a20','#2d2118'] },
    desc:'Supple leather boots, plainly made for someone who does not walk to war.' },

  gold_ring:       { name:'Gold Ring', icon:'💍', rarity:'fine', value:55,
    unique:false, slot:'ring', fx:{ dip:1, prestige:0.5 }, qualityFx:{ dip:1 },
    art:{ kind:'ring', metals:['#d1ad47','#e0c060'], gems:['#7b262d','#2f5e83'] },
    desc:'Gold says what a common tongue might stammer.' },
  ...
  engraved_armilla:{ name:'Engraved Armilla', icon:'💍', rarity:'fine', value:65,
    unique:true, slot:'ring', fx:{ prestige:1, dip:1 },
    art:{ kind:'ring', metals:['#d1ad47'], gems:['#2f5e83'], seal:true },
    desc:'An arm-ring cut with the tallies of some forgotten generosity.' },
  cipher_disk:     { name:'Cipher Disk', icon:'⚙', rarity:'fine', value:60,
    unique:true, slot:'hand', ageMin:10, fx:{ int:2 },
    art:{ kind:'pendant', metals:['#a8843c','#8f7236'] },
    desc:'Brass rings within rings. What it cannot open, it can usually read.' },
  illuminated_codex:{ name:'Illuminated Codex', icon:'📜', rarity:'famed', value:200,
    unique:true, slot:'hand', ageMin:6, fx:{ lea:3, piety:1 },
    art:{ kind:'book', covers:['#5e2e3a'], pages:['#e8dcb2'], gems:['#b89042'] },
    desc:'A generation of monks died inside these margins, and left the light on.' },
  torc_of_old_kings:{ name:'Torc of the Old Kings', icon:'⭕', rarity:'famed', value:190,
    unique:true, slot:'neck', fx:{ prestige:2, dip:1 },
    art:{ kind:'pendant', metals:['#d8b455','#c8a04a'], cords:['#4a3021'] },
    desc:'Buried with one dynasty, dug up by another. Gold keeps no grudges.' },

  /* Legendary and sacred artifacts — unique, eventOnly, and gated by faith,
     culture, and de jure region through the `artifact` field. Each exists once
     per save (state.artifacts), carries a double-edged effect profile, and can
     be permanently lost through the ordinary ownership exits. They never enter
     peddler, loot, auction, or spoil pools. See docs/designs/items.md. */
  excalibur:       { name:'Excalibur', icon:'⚔', rarity:'legendary', value:800,
    unique:true, eventOnly:true, slot:'hand', ageMin:14,
    artifact:{ religionGroups:['christian'], empires:['e_britannia'] },
    fx:{ mar:3, battle:0.05, prestige:2, health:-0.004 },
    art:{ kind:'sword', metals:['#e8ecea'], grips:['#2e4a5e'], gems:['#7fa8c8'], runes:true },
    desc:'The sword the lake gave back once, and will not give back again. It draws battle to its bearer like a candle draws moths.' },
  tyrfing:         { name:'Tyrfing', icon:'🗡', rarity:'legendary', value:700,
    unique:true, eventOnly:true, slot:'hand', ageMin:14,
    artifact:{ religionGroups:['norse_pagan'], empires:['e_scandinavia'] },
    fx:{ mar:3, battle:0.06, piety:-2, health:-0.006 },
    art:{ kind:'sword', metals:['#b8bcbc'], grips:['#4a1e1e'], gems:['#8b2635'], runes:true },
    desc:'Forged by dwarves under duress, cursed to drink a life whenever it is drawn. It has never missed.' },
  gungnir:         { name:'Gungnir', icon:'🔱', rarity:'legendary', value:750,
    unique:true, eventOnly:true, slot:'hand', grip:2, ageMin:14,
    artifact:{ religionGroups:['norse_pagan'], empires:['e_scandinavia'] },
    fx:{ battle:0.05, mar:2, dip:-1 },
    art:{ kind:'spear', woods:['#3a2c1e'], metals:['#d8dcd8'], runes:true },
    desc:'The Allfather’s spear never misses its mark — and its bearer’s oaths are watched as closely as his throws.' },
  mjolnir_amulet:  { name:'Mjölnir Amulet', icon:'⚡', rarity:'legendary', value:500,
    unique:true, eventOnly:true, slot:'neck',
    artifact:{ religionGroups:['norse_pagan'], empires:['e_scandinavia'], sacred:true },
    fx:{ piety:2, battle:0.02, dip:-1 },
    art:{ kind:'pendant', metals:['#8a9090','#6a7272'], cords:['#3a2c1e'] },
    desc:'Thor’s hammer in iron miniature, heavy with thunder. Priests of the White Christ spit when they see it.' },
  durendal:        { name:'Durendal', icon:'⚔', rarity:'legendary', value:700,
    unique:true, eventOnly:true, slot:'hand', ageMin:14,
    artifact:{ religionGroups:['christian'], empires:['e_francia'], sacred:true },
    fx:{ mar:2, battle:0.04, piety:1, gold:-2 },
    art:{ kind:'sword', metals:['#dfe3dd'], grips:['#3a3428'], gems:['#b89042'] },
    desc:'Roland’s sword, relics sealed in its golden hilt. Keeping them housed and honored is not cheap.' },
  zulfiqar:        { name:'Zulfiqar', icon:'🗡', rarity:'legendary', value:800,
    unique:true, eventOnly:true, slot:'hand', ageMin:14,
    artifact:{ religionGroups:['muslim'], kingdoms:['k_arabia','k_iraq','k_jazira','k_syria'], sacred:true },
    fx:{ mar:2, battle:0.04, prestige:2, dip:-1 },
    art:{ kind:'sword', metals:['#e2e6e2'], grips:['#213f4b'], pattern:'watered' },
    desc:'The forked blade given to Ali beneath the Prophet’s own banner. Every jurist has an opinion about whose hand may hold it.' },
  holy_lance:      { name:'The Holy Lance', icon:'⚜', rarity:'legendary', value:900,
    unique:true, eventOnly:true, slot:'neck',
    artifact:{ religionGroups:['christian'], empires:['e_byzantium'], sacred:true },
    fx:{ piety:3, battle:0.03, dip:-1 },
    art:{ kind:'relic', metals:['#8a9090','#6a7272'], gems:['#7f2735'], glow:true },
    desc:'The iron that pierced the side at Golgotha, bound in gold. Every prelate in Christendom claims the right to guard it.' },
  true_cross_fragment:{ name:'Fragment of the True Cross', icon:'✝', rarity:'legendary', value:800,
    unique:true, eventOnly:true, slot:'neck',
    artifact:{ religionGroups:['christian'], kingdoms:['k_syria'], sacred:true },
    fx:{ piety:3, prestige:1, gold:-2 },
    art:{ kind:'relic', woods:['#5a3a24'], metals:['#d3ae4a'], glow:true },
    desc:'A splinter of the wood itself, carried out of Jerusalem’s treasuries. Pilgrims will come, and pilgrims must be fed.' },
  book_of_kells:   { name:'The Book of Kells', icon:'📕', rarity:'legendary', value:600,
    unique:true, eventOnly:true, slot:'hand', ageMin:6,
    artifact:{ religionGroups:['christian'], cultures:['gaelic'], kingdoms:['k_ireland'], sacred:true },
    fx:{ lea:3, piety:2, prestige:1, mar:-1 },
    art:{ kind:'book', covers:['#3a5e46'], pages:['#e8dcb2'], gems:['#b89042'] },
    desc:'Columba’s monks poured a whole civilization into these pages. A house that studies them grows wise, and soft.' },
  ring_of_solomon: { name:'The Ring of Solomon', icon:'💍', rarity:'legendary', value:700,
    unique:true, eventOnly:true, slot:'ring',
    artifact:{ religionGroups:['jewish','muslim'], kingdoms:['k_syria','k_arabia','k_iraq'], sacred:true },
    fx:{ dip:2, lea:2, int:2, piety:-1 },
    art:{ kind:'ring', metals:['#c8b678'], gems:['#2e3a4a'], seal:true },
    desc:'The seal that commanded kings, spirits, and beasts. The devout lower their voices around it — and around you.' },
  derafsh_kaviani: { name:'The Derafsh Kaviani', icon:'🚩', rarity:'legendary', value:750,
    unique:true, eventOnly:true, slot:'hand', grip:2, ageMin:12,
    artifact:{ religionGroups:['zoroastrian'], empires:['e_persia'], sacred:true },
...
FBDATA.balance = {
  ...
  /* market stall / bazaar shop (js/items.js FB.shopStock): town and city
     settlement visits can open a small seasonal stock of ordinary gear,
     rerolled whenever the season, county, or settlement kind changes.
     Quality odds are curated above the loot table's 70/25/5; the urban_markets
     technology widens the stock and shifts quality upward. Prices pass the
     county market quote and the always-available convenience ratio. */
  shopStockSize: { town:[4,6], city:[6,9] },
  shopQualityOdds: { plain:55, well:32, masterwork:13 },
  shopQualityOddsTech: { plain:40, well:40, masterwork:20 },
  shopStockTechBonus: 2, // extra offers when the nation knows urban_markets
  shopPriceRatio: 1.1, // always-available convenience over peddler scarcity
  ...
  armyProvisionPrice: 0.1125, // rescaled with physical food units; local prices still apply
  armyProvisionMenPerUnit: 30, // infantry fed for a season by one market unit
  ...
  governmentAdministrationRate: 0.50,
  governmentCourtRate: 0.10,
  governmentScaleRate: 0.05,
  governmentSeatCost: 0.5,
  governmentCourtAllowance: [1, 2, 4, 8, 16],
  realmReserveSeasons: 10,
  realmReserveRevenueFloor: 0.50,
  distributionMinimum: 10,
  distributionSurplusRate: 0.10,
  distributionCooldownDays: 360,
  fieldNonFoodCostMultiplier: 3,
  ...
  cultureAdoptionSelfPrestige: 150,
  marriageCulturePersuasionStanding:60,
  marriageCulturePersuasionHeirStanding:80,
  marriageCulturePersuasionBase:0.25,
  marriageCulturePersuasionStandingRate:0.005,
  marriageCulturePersuasionDiplomacyRate:0.02,
  marriageCulturePersuasionMin:0.05,
  marriageCulturePersuasionMax:0.60,
  marriageCulturePersuasionHeirMult:0.5,
  marriageCulturePersuasionHeirCap:0.25,
  marriageCulturePersuasionRefusalStanding:10,
  cultureAdoptionHouseholdPrestige: 450, cultureAdoptionHouseholdPiety: 150,
  ...
  countyCommunityProjectBaseRate: 0.012,
  countyCommunityProjectMaxRate: 0.015,
  countyCommunityProjectMinTransfer: 25,
  countyCommunityProjectPopulationReference: 24000,
  countyCommunityProjectPopulationScaleMin: 0.65,
  countyCommunityProjectPopulationScaleMax: 1.35,
  ...
  countyCommunityProjectEntrenchmentMax: 0.30,
  countyCommunityProjectUnrestResistance: 0.35,
  countyCommunityProjectResistanceCap: 0.85,
  countyCommunityProjectWarMultiplier: 0.50,
  countyCommunityProjectOccupationMultiplier: 0.15,
  countyCommunityProjectFaithResistance:{
    same:0, in_fold:0.08, schismatic:0.16, foreign:0.28, hostile:0.42
  },
  countyCommunityProjectCultureResistance:{ same_group:0.10, foreign:0.32 },
  countyCommunityProjectPolicies:{
    voluntary:{ pressure:0.70, resistance:0.65, maxRate:0.006,
      holdout:0.05, migration:0 },
    integrative:{ pressure:1.00, resistance:0.90, maxRate:0.012,
      holdout:0.12, migration:0 },
  ...
  cultureAdoptionHouseholdRealmStanding: -15,
  cultureAdoptionVassalStanding: -25,
  ...
  enterpriseChainBonus: 0.5, // a chain consumer (def.chainFrom) yields this much more while a household input enterprise produces in the same province

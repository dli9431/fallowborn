...
test.describe('County Population & Lightweight Demographics Engine', () => {
  test('Deterministic opening baseline fallback and bookmark overrides', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.countyPopulationBaseline);

    const results = await page.evaluate(() => {
      const state = FB.state || {};
      const provs = FB.world.provs.filter(p => !p.wasteland);
      ...
      for (let i = 0; i < Math.min(10, provs.length); i++) {
        ...
        sample.push({ pid: pr.id, dev0, terrain: pr.terrain, base, expected });
      }

      return { allAboveFloor, matchesMath, sample };
    });
  ...
  test('Carrying capacity responds to buildings and technology with caps', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.countyPopulationCapacity);
  ...
  test('Natural growth exhibits logistic pressure and clamps', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.populationYear);
  ...
  test('Conserved land migration maintains zero world-sum delta and respects limits', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.populationYear);
  ...
  test('Population factor scales tax, levies, and market demand within 0.50 - 1.50 range', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.countyPopulationFactor);
    ...
    const factorData = await page.evaluate(() => {
      ...
      return { factorBase, factorHigh, factorLow };
    });
  ...
  test('Fort tiers mitigate hostile siege capture losses', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.damageCountyPopulation);

    const siegeData = await page.evaluate(() => {
      const state = {
        turn: 1,
        date: { year: 867 },
        forts: {},
      ...
      state.forts[pid] = { level: 4, ruined: false };
      state.population.counties[pid] = { count: basePop, natural: 0, migration: 0, losses: 0 };
      const lossTier4 = FB.damageCountyPopulation(state, pid, 'capture');

      return { lossTier0, lossTier4 };
    });
  ...
  test('Settlement population allocations sum exactly to county population', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.settlementPopulations);

    const settlementResult = await page.evaluate(() => {
      const state = {
        turn: 1,
        date: { year: 867 },
        population: { schema: 1, lastYear: 867, counties: {} }
      ...
      const provs = FB.world.provs.filter(p => !p.wasteland && FB.settlementsOf(state, p.id).length > 1);
      ...
      for (let i = 0; i < Math.min(10, provs.length); i++) {
        ...
        samples.push({ pid: pr.id, total, allocations, sum });
      }

      return { allSumExact, samples };
    });

    expect(settlementResult.allSumExact).toBe(true);
  });

  test('Lazy save migration backfills population with dev scaling and building bonuses', async ({ page }) => {
    await page.goto('index.html');
    await page.waitForFunction(() => window.FB && window.FB.ensurePopulationState);

...
Kept in ignored `notes/` because it records private save/message forward constraints.
...
(and, under future multiplayer, per client). `FB.newsText` renders either — `msg` through
the localization layer, else the frozen `t` — so old saves and unstructured third-party mod
calls keep working with no migration. Every core `FB.news` producer now uses the structured
form. `js/messages.js` clones, validates, and freezes JSON-safe semantic params at the
boundary; state never stores an active-locale rendering for a new core chronicle entry.
...
`state.seed` records the start code the life began with
([seeds.md](../docs/designs/seeds.md)); saves
from before it existed simply hide the seed row in the menu.

Related: [mods.md](../docs/designs/mods.md) for how saves are stamped with the active mod set.

...
liege_grant war_battle plot appeal_outcome vassal_comply county_petition parliament_vote
travel_trade`) with `success` / `failure` branches (`{text, effects}`), and `effects`.

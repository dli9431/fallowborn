...
**Continue** action. The settled result and participant portraits remain visible.
All result chips, including actual costs, losses, gains, and status changes, use
the shared Details desktop hover/focus tooltip or compact 44px question-mark
disclosure. No result-chip row appears directly on a modal face; exact decision
costs remain visible before commitment.
...
`.event-details-button`. Entering a disclosure layout, or interacting with one of its cards,
immediately clears any open side tooltip so it cannot cover or intercept compact controls.
On phone and short-screen
...
longer create a blocking outcome screen unless they settle a significant milestone; one receipt toast appears for six seconds above
...
Freedom sheets align each portrait beside its name. Optional relatives use one checkbox,
portrait, and stacked name/price row. Blocked social access shows the missing station
inline and links an existing, reachable local intermediary with a portrait. No person
is generated by opening this guidance. Read-only event Details controls remain usable
during the brief input guard; committing and acknowledging choices stay guarded.

Outcome sheets do not suppress or remove result toasts. Freedom uses one sentence describing the new station and opportunities; exact consequences remain in the Details disclosure.

Outcome screens omit the generic "Resolved: [choice]" line and empty "It goes well/poorly" prose. Authored aftermaths lead with the settled result; shared chips retain exact consequences inside Details. The neutral heading is "Outcome".
...
Actual money, prestige, and piety changes use settled consequence chips inside the
shared Details tooltip/disclosure, including generic promotion and inline uprising results.
Inline uprising results return to the privilege list with scroll and disclosures intact.

Final petitions and uprising notices name affected counties once, retain the response window or current disruption and spread risk, and use short sentence-case titles. Concession disclosures group identical county terms once beside the complete county list; the underlying per-county impact ledger stays intact. Bishop appointment actions form a spaced full-width stack, including wrapped cost/chance labels and Back.
